...
The RPM spec file provided here is for Bull-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bull.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bull git repository](https://github.com/BullProject/Bullcoin) but are not part of
...
Currently only `Source10` is used, for the example `bull.conf` file.
...
in the bull git repository. At present only an SVG version of the Bull
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bull
...
needed when 0.12.1 is released, a proper fix is already in the Bull git
master branch.
...
idea to build Bull Core against a static version of that library compiled
...
The `bull.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bull-server` package. At this point, most RPM based
...
The `bull-server` package should have SELinux support. How to properly do
...
Bull-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bull/). He has been
packaging Bull for Fedora at least since 2012.
...
4. I renamed the `bull` package that contains the Qt GUI to `bull-core` as
that appears to be how the general population refers to it, in contrast to
`bull-xt` or `bull-classic`. I wanted to make sure the general population
